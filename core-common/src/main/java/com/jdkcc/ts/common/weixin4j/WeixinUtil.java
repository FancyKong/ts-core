package com.jdkcc.ts.common.weixin4j;
...
public class WeixinUtil {
	...
	public static OAuthInfo getOAuthOpenid(String code) {
		...
		try {
			...
			oAuthInfo = JSON.parseObject(EntityUtils.toString(entity,"UTF-8"),
					OAuthInfo.class);
		} catch (Exception e) {
	...
	public static UserInfo getUserInfo(String openid,String accessToken) {
		...
		try {
			...
			userInfo = JSON.parseObject(EntityUtils.toString(entity,"UTF-8"),
					UserInfo.class);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
